    ...
    class Envato_Theme_Setup_Wizard {
        ...
        public function init_globals() {
            ...
            $this->site_styles = array(
                ...
                'hireright' => 'Hireright',
                'jobshub' => 'Jobshub',
                'belovedjobs' => 'Beloved Jobs',
                'jobsonline' => 'Jobs Online',
                'homejob' => 'Home Job',
                'jobfinder' => 'Jobs Finder',
                'rtl' => 'Arabic RTL',
            );
            ...
            $this->plugin_url = careerfy_framework_get_url('envato_setup');
        }
        ...
        private function _content_default_get() {
            ...
            if (class_exists('RevSlider') && ($theme_style_name == 'belovedjobs' || $theme_style_name == 'jobsonline' || $theme_style_name == 'findperson' || $theme_style_name == 'perfectjob')) {
                $content['revslider'] = array(
                    'title' => esc_html__('Revolution Slider'),
                    'description' => esc_html__('Insert demo Slider.'),
                    'pending' => esc_html__('Pending.'),
                    'installing' => esc_html__('Installing Slider.'),
                    'success' => esc_html__('Success.'),
                    'install_callback' => array($this, '_content_rev_slider_install'),
                    'checked' => $this->is_possible_upgrade() ? 0 : 1,
                        // dont check if already have content installed.
                );
            }
            ...
            $content['theme_options'] = array(
                ...
                'description' => esc_html__('Configure Theme Options.'),
                'pending' => esc_html__('Pending.'),
                'installing' => esc_html__('Installing Theme Options.'),
                'success' => esc_html__('Success.'),
                'install_callback' => array($this, '_theme_options_install'),
                'checked' => $this->is_possible_upgrade() ? 0 : 1,
                    // dont check if already have content installed.
            );
            ...
            $content['wp_bakery_templates'] = array(
                'title' => esc_html__('WpBakery Templates'),
                'description' => esc_html__('Import WpBakery Templates.'),
                'pending' => esc_html__('Pending.'),
                'installing' => esc_html__('Installing WpBakery Templates.'),
                'success' => esc_html__('Success.'),
                'install_callback' => array($this, '_content_wpbakery_templates'),
                'checked' => $this->is_possible_upgrade() ? 0 : 1,
                    // dont check if already have content installed.
            );
        ...
        private function _content_rev_slider_install() {
            ...
            if (function_exists('careerfy_demo_data_get_path')) {
                $theme_style = careerfy_demo_data_get_path('content/' . basename(get_theme_mod('dtbwp_site_style', $this->get_default_theme_style())) . '/');
                $slider_array = array($theme_style . "home-slider.zip");
        ...
        private function _fetch_remote_file($url, $post) {
            ...
            if (function_exists('careerfy_demo_data_get_path')) {
                $local_file = careerfy_demo_data_get_path('content/images/') . $file_name;
            } else {
                $local_file = careerfy_framework_get_path('envato_setup/content/images/stock/') . $file_name;
            }
            $upload = false;
        ...
        public function _content_wpbakery_templates() {
            ...
            if (function_exists('careerfy_demo_data_get_path')) {
                ...
                $wp_bakery_dir = careerfy_demo_data_get_path('content/') . 'wp_bakery_templates/';
                if (is_dir($wp_bakery_dir)) {
                    $template_files_list = $wp_filesystem->dirlist($wp_bakery_dir);
                    if (!empty($template_files_list)) {
                        $theme_mods = get_option('wpb_js_templates');
                        if (empty($theme_mods)) {
                            $theme_mods = array();
                        }
                        foreach ($template_files_list as $template_json_file) {
                            if (isset($template_json_file['name']) && file_exists($wp_bakery_dir . $template_json_file['name'])) {
                                $templt_act_file = $wp_bakery_dir . $template_json_file['name'];
                                $file_contents = $wp_filesystem->get_contents($templt_act_file);
                                $file_contents = json_decode($file_contents, true);

                                // Mergers new options and clean
                                $import_templates = array();
                                if (!empty($file_contents) && is_array($file_contents)) {
                                    $new_templte_id = uniqid('Template_');
                                    $import_templates[$new_templte_id] = $file_contents;
                                }
        ...
        public function _theme_options_install() {

            $this->_handle_delayed_posts(true); // final wrap up of delayed posts.
            $this->vc_post(); // final wrap of vc posts.

            $custom_options = $this->_get_json('theme_options.json');

            if (isset($custom_options['careerfy_framework_options']) && class_exists('ReduxFrameworkInstances')) {
                $optionsss = json_decode($custom_options['careerfy_framework_options'], true);
                $redux = ReduxFrameworkInstances::get_instance('careerfy_framework_options');
                $redux->set_options($optionsss);
            }
        ...
        public function _get_json($file, $path = '') {
            ...
            if (is_file($theme_style . basename($file))) {
                ...
                if (file_exists($file_name)) {
                    ...
                    if (strpos($file_content, 'https:\\\/\\\/careerfy.net\\\/' . basename(get_theme_mod('dtbwp_site_style', $this->get_default_theme_style())) . '\\\/') !== false) {
                        $find_url = 'https:\\\/\\\/careerfy.net\\\/' . basename(get_theme_mod('dtbwp_site_style', $this->get_default_theme_style())) . '\\\/';
                        $file_content = str_replace(array($find_url), array($replace_url_with), $file_content);
                    } else if (strpos($file_content, 'https:\\\/\\\/careerfy.net\\\/demo\\\/') !== false) {
                        $find_url = 'https:\\\/\\\/careerfy.net\\\/demo\\\/';
                        $file_content = str_replace($find_url, $replace_url_with, $file_content);
                    }
                    $replace_url_with = str_replace('/', '\/', home_url('/'));
                    if (strpos($file_content, 'https:\/\/careerfy.net\/' . basename(get_theme_mod('dtbwp_site_style', $this->get_default_theme_style())) . '\/') !== false) {
                        $find_url = 'https:\/\/careerfy.net\/' . basename(get_theme_mod('dtbwp_site_style', $this->get_default_theme_style())) . '\/';
