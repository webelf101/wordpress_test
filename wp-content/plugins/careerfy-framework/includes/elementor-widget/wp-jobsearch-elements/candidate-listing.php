...
class CandidateListings extends Widget_Base
{
    ...
    public function jobsearch_candidates_content($candidate_arg = '')
    {
        ...
        global $wpdb, $jobsearch_form_fields, $jobsearch_search_fields, $pagenow, $sitepress, $jobsearch_plugin_options;
...
//            var_dump($candidate_loop_obj->request);
        ...
        <form id="jobsearch_candidate_frm_<?php echo absint($candidate_short_counter); ?>">
            ...
            <div class="jobsearch-row">
                ...
                if (($candidate_filters_sidebar == 'yes') || (!empty($jobsearch_candidate_sidebar))) {
    ...
    protected function render()
    {
        ...
        if ($view_candidate === false) {
            ...
            $restrict_msg = isset($jobsearch_plugin_options['restrict_cand_msg']) && $jobsearch_plugin_options['restrict_cand_msg'] != '' ? $jobsearch_plugin_options['restrict_cand_msg'] : esc_html__('The Page is Restricted only for Subscribed Employers', 'careerfy-frame');
            ?>
            <div class="jobsearch-column-12">
                <div class="restrict-candidate-sec">
                    ...
                    <?php
                    if ($is_employer) { ?>
                        <p><?php esc_html_e('Please buy a C.V package to view this candidate.', 'careerfy-frame') ?></p>
                        <?php
                    } else if (is_user_logged_in()) {
                        ?>
                        <p><?php esc_html_e('You are not an employer. Only an Employer can view a candidate.', 'careerfy-frame') ?></p>
                        <?php
                    } else {
                        ?>
                        <p><?php esc_html_e('If you are employer just login to view this candidate or buy a C.V package to download His Resume.', 'careerfy-frame') ?></p>
                        <?php
                    }
                    if (is_user_logged_in()) { ?>
                        <div class="login-btns">
                            <a href="<?php echo wp_logout_url(home_url('/')); ?>"><i
                                        class="jobsearch-icon jobsearch-logout"></i><?php esc_html_e('Logout', 'careerfy-frame') ?>
                            </a>
                        </div>
                        <?php
                    } else {
                ...
                if (!empty($restrict_cv_pckgs) && is_array($restrict_cv_pckgs) && $restrict_candidates_for_users == 'register_resume') {
                    ...
                    <div class="cv-packages-section">
                        <div class="packages-title">
                            <h2><?php esc_html_e('Buy any CV Packages to get started', 'careerfy-frame') ?></h2></div>
                        <?php
                        ob_start();
                        ?>
                        <div class="jobsearch-row">
                                            ...
                                            <div class="grab-classic-priceplane">
                                                ...
                                                <?php if (is_user_logged_in()) { ?>
                                                    <a href="javascript:void(0);"
                                                       ...
                                                       data-id="<?php echo($cv_pkg_id) ?>"><?php esc_html_e('Get Started', 'careerfy-frame') ?> </a>
                                                    <span class="pkg-loding-msg" style="display:none;"></span>
                                                <?php } else { ?>
                                                    <a href="javascript:void(0);"
                                                       class="jobsearch-classic-priceplane-btn jobsearch-open-signin-tab"><?php esc_html_e('Get Started', 'careerfy-frame') ?> </a>
                                                <?php } ?>
                                            </div>
                                        </div>
                                    </div>
