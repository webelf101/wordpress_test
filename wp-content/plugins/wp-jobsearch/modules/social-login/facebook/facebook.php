...
class JobsearchFacebook
{
    ...
    public function init_loaded()
    {
        ...
        if (isset($_GET['action']) && $_GET['action'] == 'jobsearch_facebook' && session_status() == PHP_SESSION_NONE) {
            session_start();
        }
    }
    ...
    public function do_apply_job_with_facebook($user_id)
    {
        ...
        if ($apply_job_cond && isset($_COOKIE['jobsearch_apply_fb_jobid']) && $_COOKIE['jobsearch_apply_fb_jobid'] > 0) {
    ...
    public function apply_job_with_fb($args = array())
    {
        ...
        if ($this->app_id != '' && $this->app_secret != '' && $facebook_login == 'on') {
            ...
            <?php } elseif ($view == 'job4') { ?>
                <a href="javascript:void(0);" class="<?php echo($classes); ?>"
                   data-id="<?php echo($job_id) ?>"> <?php esc_html_e('Apply with with Facebook', 'wp-jobsearch') ?></a>
