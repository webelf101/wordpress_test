...
Class WpJobSearchLogin
{
    ...
    public function init_loaded()
    {
        ...
        if (isset($_GET['action']) && $_GET['action'] == 'linkedin_login' && session_status() == PHP_SESSION_NONE) {
            session_start();
        }
    }
    ...
    public function do_apply_job_with_linkedin($user_id)
    {
        ...
        if ($apply_job_cond && isset($_COOKIE['jobsearch_apply_linkedin_jobid']) && $_COOKIE['jobsearch_apply_linkedin_jobid'] > 0) {
    ...
    public function apply_job_with_linkedin($args = array())
    {
        ...
        if ($linkedin_login == 'on') {
            ...
            <?php } elseif ($view == 'job4') { ?>
                <a href="javascript:void(0);" class="<?php echo($classes); ?>"
                   data-id="<?php echo($job_id) ?>"> <?php esc_html_e('Apply with Linkedin', 'wp-jobsearch') ?></a>
            <?php } elseif ($view == 'job5') { ?>
                <a href="javascript:void(0);" class="<?php echo($classes); ?>" data-id="<?php echo($job_id) ?>"><i
                            class="careerfy-icon careerfy-linkedin1"></i> <?php esc_html_e('Apply with Linkedin', 'wp-jobsearch') ?>
                </a>
