    ...
    class Jobsearch_Shortcode_Jobs_Frontend
    {
        ...
        public function jobsearch_quick_job_popup_load()
        {
            ...
            <div class="jobsearch-quick-apply-box-inner">
                <div class="jobsearch-quick-apply-box-left">
                    <?php $Jobsearch_QuickJobApplyLoad::jobsearch_quick_apply_job_detail_content($job_id); ?>
                </div>
                <div class="jobsearch-quick-apply-box-right <?php echo $is_not_candidate == true || $is_applied == true ? 'jobsearch-apply-box-right-validate' : ''; ?>">
                    <?php $Jobsearch_QuickJobApplyLoad::quick_job_application_popup_form($job_id, '250'); ?>
                </div>
            </div>
...
//            );
            ...
            if (isset($_REQUEST['sector_cat']) && $_REQUEST['sector_cat'] != '') {
                $sec_terms_arr = array();
                $sec_terms_str = $_REQUEST['sector_cat'];
                                ...
                                if (is_array($qry_val)) {
                                    foreach ($qry_val as $qry_val_var => $qry_val_value) {
                                        if ($qry_val_value != '') {
                                            ...
                                            $keyword_html .= '<a href="' . remove_query_arg('lang', jobsearch_remove_qrystr_extra_var($qrystr1, $qry_var)) . '" title="' . ucwords(str_replace(array("+", "-", "_"), " ", str_replace('jobsearch_field_', '', $qry_var))) . '">' . stripslashes($qry_val_str) . ' <i class="fa fa-window-close"></i></a>';
                                            $keyword_html .= '</li>';
                                        }
                                    }
                                } else {
                                    ...
                                    $keyword_html .= '<li>';
                                    $keyword_html .= '<a href="' . remove_query_arg('lang', jobsearch_remove_qrystr_extra_var($qrystr, $qry_var)) . '" title="' . ucwords(str_replace(array("+", "-", "_"), " ", str_replace('jobsearch_field_', '', $qry_var))) . '">' . stripslashes($qry_val_str) . ' <i class="fa fa-window-close"></i></a>';
                                    $keyword_html .= '</li>';
                                }
                            }
                        }
