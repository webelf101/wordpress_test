...
function jobsearch_user_account_links($args = array())
{
    ...
    if ($login_myacount_btns != 'off') {
        ...
        $account_register_icon = isset($header_style) && $header_style == 'style21' ? '<i class="careerfy-icon careerfy-user-1"></i>' : '';
        ...
        if (is_user_logged_in()) {
            ...
            if (isset($user_dashboard_page) && $user_dashboard_page != '') {
                ...
                <li class="jobsearch-userdash-menumain">
                    <?php if ($header_style == 'style2') { ?>
                        <a href="<?php echo esc_url(get_permalink($user_dashboard_page)) ?>"
                           class="careerfy-btn-icon jobsearch-open-signin-tab"><i
                                    class="careerfy-icon careerfy-user"></i></a>
                    <?php } else { ?>
                        <a href="<?php echo esc_url(get_permalink($user_dashboard_page)) ?>"
                           class="<?php echo $account_link_btn_color_class ?> <?php echo $active_link_class ?>  active"><?php echo apply_filters('jobsearch_header_user_myaccount_txt', esc_html__('My Account', 'wp-jobsearch')); ?></a>
                    <?php } ?>
            ...
            if ($is_popup) {
                ...
                <?php } ?>
                <li><a href="javascript:void(0);"
                       class="<?php echo $account_link_btn_color_class ?> jobsearch-open-signin-tab <?php echo $active_link_class ?>"><?php echo $account_link_icon ?><?php echo esc_html__($account_link_text, 'wp-jobsearch'); ?></a>
                </li>
                <?php if ($header_style == 'style22') { ?>
                    <li class="jobsearch-logto-link"><a
                                href="javascript:void(0);" class="jobsearch-open-register-tab"><i class="careerfy-icon careerfy-user-1"></i> <?php echo esc_html__('Sign Up', 'wp-jobsearch'); ?></a>
                    </li>
                <?php } ?>
            <?php } else {
                if ($register_link_view === true) { ?>
                    <li class="jobsearch-regto-link"><a
                                href="<?php echo esc_url(get_permalink($jobsearch_registr_page)) ?>"><?php echo esc_html__('Register', 'wp-jobsearch'); ?></a>
                    </li>
                <?php } ?>
                <li class="jobsearch-logto-link"><a
                            href="<?php echo esc_url(get_permalink($jobsearch_login_page)) ?>"> <?php echo esc_html__('Sign In', 'wp-jobsearch'); ?></a>
                </li>
                <?php
            }
